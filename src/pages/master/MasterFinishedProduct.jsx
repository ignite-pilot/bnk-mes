...
const API = '/api/master-finished-products';
...
function formatMm(v) { if (v == null || v === '') return '-'; const n = Number(v); return Number.isNaN(n) ? '-' : `${n % 1 === 0 ? n : n.toFixed(2)}mm`; }
...
function MasterFinishedProduct() {
  const { user } = useAuth();
  ...
  const handleExcelDownload = async () => {
    ...
    try {
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'master_finished_products.csv'; a.style.display = 'none';
          ...
          {uploadResult.errors.length > 0 && (
            <ul className={styles.errorBlockerList}>
              {uploadResult.errors.map((e, i) => (
                <li key={i} className={styles.errorBlockerItem}>{e.row}행 [{e.code || e.name || e.semiType}]: {e.errors.join(', ')}</li>
              ))}
            </ul>
          )}
              ...
              {list.length === 0 ? (
                ...
                <tr key={row.id} onClick={() => openView(row.id)} style={{ cursor: 'pointer' }}>
                  ...
                  {!isMobile && <>
                    ...
                    <EllipsisCell>{formatNum(row.two_width)}</EllipsisCell>
        ...
        <div className={styles.modalOverlay} onClick={closeForm} role="presentation">
          <div className={styles.modal} onClick={e => e.stopPropagation()} role="dialog" aria-modal="true">
            <h2 className={styles.modalTitle}>
              {formMode === 'add' && '완제품 등록'}{formMode === 'view' && '완제품 보기'}{formMode === 'edit' && '완제품 수정'}
            </h2>
            ...
            {formMode === 'view' ? (
              <div className={styles.viewForm}>
                <div className={styles.dl} style={{ gridTemplateColumns: '120px 1fr', margin: 0, borderRadius: 8, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                  <dt>완제품 코드</dt><dd style={{ fontFamily: 'monospace' }}>{renderCell(formData.code)}</dd>
              ...
              <form onSubmit={formMode === 'add' ? handleSubmitAdd : handleSubmitEdit} className={styles.form}>
                <label className={styles.label}>완제품 코드<input type="text" value={formData.code || ''} onChange={e => setFormData(f => ({ ...f, code: e.target.value }))} className={styles.input} /></label>
                ...
                <label className={styles.label}>두폭<input type="number" step="any" min="0" value={formData.two_width} onChange={e => setFormData(f => ({ ...f, two_width: e.target.value }))} className={styles.input} /></label>
...
export default MasterFinishedProduct;
