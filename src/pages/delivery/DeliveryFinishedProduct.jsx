...
function DeliveryFinishedProduct() {
  ...
  const handleSubmitAdd = async (e) => {
    ...
    if (!formData.name?.trim()) {
      setFormError('완제품 이름은 필수입니다.');
      return;
    }
    if (!formData.code?.trim()) {
      setFormError('완제품 코드는 필수입니다.');
      return;
    }
    setFormSaving(true);
    setFormError('');
              ...
              {list.length === 0 ? (
                ...
                list.map((row) => (
                  <tr key={row.id}>
                    ...
                    {!isMobile && (
                      <>
                        <td>{renderCell(row.vehicle_name)}</td>
                        <td>{renderCell(row.part_name)}</td>
          ...
          >
            ...
            {formMode === 'add' && (
              <form onSubmit={handleSubmitAdd} className={styles.form}>
                ...
                <label className={styles.label}>
                  완제품 이름 <span className={styles.required}>(필수)</span>
                  <input
                    type="text"
                    value={formData.name}
                ...
                <label className={styles.label}>
                  완제품 코드 <span className={styles.required}>(필수)</span>
                  <input
                    type="text"
                    value={formData.code}
            ...
            {formMode === 'edit' && (
              <form onSubmit={handleSubmitEdit} className={styles.form}>
                <label className={styles.label}>
                  완제품 이름 <span className={styles.required}>*</span>
                  <input
                    type="text"
                    value={formData.name}
                    onChange={(e) => setFormData((f) => ({ ...f, name: e.target.value }))}
                ...
                <label className={styles.label}>
                  완제품 코드 <span className={styles.required}>*</span>
                  <input
                    type="text"
                    value={formData.code}
                    onChange={(e) => setFormData((f) => ({ ...f, code: e.target.value }))}
