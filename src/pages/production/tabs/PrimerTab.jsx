...
function PrimerTab({ userName }) {
  ...
  const { vehicleCodes } = useConfigCodes();
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      setUploadResult(d);
      setMessage(`업로드 완료: ${d.inserted}건 저장 / 스킵 ${d.skipped}건 / 미매칭 차종 ${d.mismatch?.vehicle || 0}건`);
      fetchList();
    } catch (err) {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem', minWidth: 1500 }}>
          <thead>
            <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              ...
              <Th align="right">배율</Th>
              <Th align="right">폭</Th>
              <Th>폼LOT</Th>
              <Th>BnK LOT</Th>
              <Th align="right">입고수량</Th>
              <Th align="right">실생산량</Th>
              <Th>이면처리</Th>
              <Th>이면처리검증</Th>
              <Th align="right">수율</Th>
              <Th align="right">수율수량</Th>
              <Th>비고</Th>
              <Th>작업</Th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr style={{ backgroundColor: '#f0fdf4', borderBottom: '1px solid #e2e8f0' }}>
              ...
              <Td><input value={newRow.div} onChange={(e) => setNewRow({ ...newRow, div: e.target.value })} style={inputStyle} placeholder="구분/품명" /></Td>
              <Td>
                <SelectDropdown
              ...
              <Td><input type="number" min="0" step="0.001" value={newRow.thickness} onChange={(e) => setNewRow({ ...newRow, thickness: e.target.value })} style={{ ...inputStyle, textAlign: 'right' }} /></Td>
              <Td><input type="number" min="0" step="0.0001" value={newRow.ratio} onChange={(e) => setNewRow({ ...newRow, ratio: e.target.value })} style={{ ...inputStyle, textAlign: 'right' }} /></Td>
              ...
              <Td><input value={newRow.foam_lot} onChange={(e) => setNewRow({ ...newRow, foam_lot: e.target.value })} style={inputStyle} /></Td>
              <Td><input value={newRow.bnk_lot} onChange={(e) => setNewRow({ ...newRow, bnk_lot: e.target.value })} style={inputStyle} /></Td>
              ...
              <Td><input value={newRow.back_treat} onChange={(e) => setNewRow({ ...newRow, back_treat: e.target.value })} style={inputStyle} /></Td>
              <Td><input value={newRow.back_treat_check} onChange={(e) => setNewRow({ ...newRow, back_treat_check: e.target.value })} style={inputStyle} /></Td>
              ...
              <Td><input type="number" min="0" value={newRow.yield_qty} onChange={(e) => setNewRow({ ...newRow, yield_qty: e.target.value })} style={{ ...inputStyle, textAlign: 'right' }} /></Td>
            ...
            {/* 조회된 데이터 */}
            {filtered.map((row) => {
              const hasMismatch = !row.vehicleMatched;
              return (
                <tr key={row.id}
                  ...
                  <Td>{row.back_treat || '-'}</Td>
                  <Td>{row.back_treat_check || '-'}</Td>
                  <Td align="right">{row.yield_rate != null ? (Number(row.yield_rate) * 100).toFixed(1) + '%' : '-'}</Td>
                  <Td align="right">{row.yield_qty?.toLocaleString() ?? '-'}</Td>
                  <Td>{row.memo || '-'}</Td>
                  <Td>
