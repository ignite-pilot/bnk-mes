/**
 * 원자재 복수 선택 레이어 팝업 (검색/필터 가능, 확인 시 선택 반영)
 * - materials: { id, name, kind? }[]
...
function RawMaterialSelectPopup({ open, onClose, materials, selectedIds = [], onConfirm, title = '원자재 선택' }) {
  ...
  const filtered = useMemo(() => {
    ...
    return materials.filter(
      (m) =>
        ...
        String(m.kind || '').toLowerCase().includes(q)
    );
  ...
  const selectAll = () => {
    setTempSelected(materials.map((m) => m.id));
  };
  ...
  return (
    <div style={popupStyle.overlay} onClick={onClose} role="presentation">
      <div style={popupStyle.box} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="raw-material-popup-title">
        <h2 id="raw-material-popup-title" style={popupStyle.title}>
          {title}
        </h2>
        <input
          type="text"
          style={popupStyle.search}
          placeholder="원자재 이름 또는 종류로 검색"
          value={searchText}
        ...
        <div style={popupStyle.listWrap}>
          {filtered.length === 0 ? (
            <div style={{ padding: '1rem', color: '#64748b', fontSize: '0.875rem' }}>
              {materials.length === 0 ? '원자재 목록을 불러오는 중...' : '검색 결과가 없습니다.'}
            </div>
          ) : (
            filtered.map((m) => (
              <label key={m.id} style={popupStyle.item}>
                <input
                  type="checkbox"
                  checked={tempSelected.includes(m.id)}
                  onChange={() => toggle(m.id)}
                />
                <span>{[m.kind, m.name].filter(Boolean).join(' / ') || m.name || `#${m.id}`}</span>
              </label>
            ))
          )}
        </div>
...
export default RawMaterialSelectPopup;
