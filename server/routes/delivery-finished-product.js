...
const LIST_SELECT = `SELECT id, name, code, car_company, vehicle_code, vehicle_name, part_code, part_name, color_code, color_name, thickness, width, \`length\`, updated_at, updated_by
  FROM \`${TABLE}\``;

/**
 * 엑셀 다운로드 (CSV)
 * GET /api/delivery-finished-products/export-excel?name=&code=
 */
router.get('/export-excel', async (req, res) => {
  try {
    ...
    const BOM = '\uFEFF';
    const header = '완제품 이름,완제품 코드,완성차 회사,차량 코드,차량 이름,부위 코드,부위 이름,색상 코드,색상 이름,두께,폭,길이,수정일자,수정자\n';
    const toCsvCell = (v) => {
      if (v == null) return '';
      const s = String(v);
 ...
 * GET /api/delivery-finished-products?name=&code=&page=1&limit=20
 */
 ...
 * 선택: car_company, vehicle_code, vehicle_name, part_code, part_name, color_code, color_name, thickness, width, length
 */
router.post('/', async (req, res) => {
  try {
    ...
    if (!name || String(name).trim() === '') {
      return res.status(400).json({ error: '완제품 이름은 필수입니다.' });
    }
    if (!code || String(code).trim() === '') {
      return res.status(400).json({ error: '완제품 코드는 필수입니다.' });
    }
    ...
    const [result] = await getPool().query(
      `INSERT INTO \`${TABLE}\` (name, code, car_company, vehicle_code, vehicle_name, part_code, part_name, color_code, color_name, thickness, width, \`length\`, updated_at, updated_by)
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    // 코드 중복 검사 (변경 시)
    if (code !== undefined) {
      if (String(code).trim() === '') return res.status(400).json({ error: '완제품 코드는 필수입니다.' });
      const [dup] = await getPool().query(
    ...
    const params = [];
    if (name !== undefined) {
      if (String(name).trim() === '') return res.status(400).json({ error: '완제품 이름은 필수입니다.' });
      updates.push('name = ?');
      params.push(String(name).trim());
    }
    if (code !== undefined) {
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const [refSupplier] = await getPool().query(
      `SELECT COUNT(*) AS cnt FROM delivery_supplier_finished_products WHERE finished_product_id = ?`,
      [id]
    );
    const [refRequest] = await getPool().query(
      `SELECT COUNT(*) AS cnt FROM delivery_request_items WHERE item_id = ? AND item_type = 'finished'`,
      [id]
    );
    if ((refSupplier[0]?.cnt || 0) > 0 || (refRequest[0]?.cnt || 0) > 0) {
      return res.status(400).json({ error: '해당 완제품을 사용하는 곳이 있어 삭제할 수 없습니다.' });
    }
    ...
    if (result.affectedRows === 0) return res.status(404).json({ error: '완제품을 찾을 수 없습니다.' });
    res.json({ ok: true });
  } catch (err) {
    logger.error('delivery-finished-product delete error', { error: err.message });
