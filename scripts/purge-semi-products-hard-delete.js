...
async function main() {
  ...
  try {
    const [delResult] = await conn.query('DELETE FROM delivery_semi_products');
    const [countRows] = await conn.query('SELECT COUNT(*) AS cnt FROM delivery_semi_products');
    console.log(JSON.stringify({ deletedRows: delResult.affectedRows, remainingRows: Number(countRows?.[0]?.cnt || 0) }, null, 2));
  } finally {
    await conn.end();
  }
}
